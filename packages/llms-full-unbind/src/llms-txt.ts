 ...
 *
 * Parses the llms-txt style format: `<doc title="..." desc="...">content</doc>`
...
/**
 * Parse llms-txt format content synchronously
 */
export function parseLlmsTxtFormat(content: string): Page[] {
  const pages: Page[] = [];

  // Reset regex lastIndex for fresh matching
  LLMS_TXT_DOC_REGEX.lastIndex = 0;

  let match: RegExpExecArray | null;
  while ((match = LLMS_TXT_DOC_REGEX.exec(content)) !== null) {
...
/**
 * Streaming parser for llms-txt format
 */
export class LlmsTxtStreamingParser implements StreamingParser {
  private buffer = "";

 ...
 *
 * Parses the format generated by vitepress-plugin-llms: Markdown pages separated by frontmatter like section
...
export class VitepressPluginLlmsStreamingParser implements StreamingParser {
  ...
  public static detect(lines: string[]): "certain" | "maybe" | "no" {
    ...
    for (
      ...
      lineIndex >= contentIndex;
      lineIndex--
    ) {
      const frontmatter = detectFrontmatterBlock(lines, lineIndex);
      if (!frontmatter) continue;
      return lineIndex === contentIndex ? "certain" : "maybe";
    }
    return "no";
  }
...
function findSeparatorFrontmatter(lines: string[]): FrontmatterBlock | null {
  ...
  for (
    let lineIndex = contentIndex + 1;
    lineIndex < lines.length;
    lineIndex++
  ) {
    const frontmatter = detectFrontmatterBlock(lines, lineIndex);
    if (!frontmatter) continue;
    return { startLine: lineIndex, ...frontmatter };
  }
  return null;
}
